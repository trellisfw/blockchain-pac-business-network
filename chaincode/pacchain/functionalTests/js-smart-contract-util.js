...
class SmartContractUtil {
  ...
  static async getConnectionProfile() {
    const connectionProfilePath = '/Users/maverickzhn/.fabric-vscode/gateways/local_fabric/local_fabric.json';
  ...
  static async submitTransaction(contractName, functionName, args, gateway) {
    ...
    let contract;
    if (contractName !== '') {
      contract = await network.getContract('pacchain', contractName);
    } else {
      contract = await network.getContract('pacchain');
    }
